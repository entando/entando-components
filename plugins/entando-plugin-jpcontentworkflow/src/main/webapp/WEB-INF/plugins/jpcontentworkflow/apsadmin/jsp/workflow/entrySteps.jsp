...
<ol class="breadcrumb page-tabs-header breadcrumb-position">
	...
	<li class="page-title-container"><s:text
			name="title.workflowManagement.editSteps" /></li>
</ol>
		...
		<table class="table table-striped table-bordered table-hover no-mb">
			...
			<tbody>
				...
				<s:if test="%{steps != null && steps.size() > 0}">
					<s:iterator value="steps" var="step" status="rowstatus">
						<tr>
							...
							<td><wpsf:textfield name="%{code + '_SEP_descr'}"
									value="%{#step.descr}"
									cssClass="form-control input-sm display-block" /></td>
							<td><wpsf:select name="%{code + '_SEP_role'}" headerKey=""
									headerValue=" - " list="roles" listKey="name"
									listValue="description" value="%{#step.role}"
									cssClass="form-control input-sm display-block" /></td>
								...
								<div class="dropdown dropdown-kebab-pf">
									<div class="btn-group btn-group-xs">
										...
										<ul class="dropdown-menu dropdown-menu-right">
											<li><wpsa:actionParam action="moveStep" var="actionName">
													<wpsa:actionSubParam name="movement" value="UP" />
													<wpsa:actionSubParam name="elementIndex"
														value="%{#rowstatus.index}" />
												</wpsa:actionParam> <wpsf:submit action="%{#actionName}"
													...
													<span class="sr-only"><s:property
															value="%{getText('label.moveUp')+': '+#step.code}" /></span>
												</wpsf:submit></li>
											...
											<li><wpsa:actionParam action="moveStep" var="actionName">
													...
													<span><s:text name="label.moveDown" /></span>
													<span class="sr-only"><s:property
															value="%{getText('label.moveDown')+': '+#step.code}" /></span>
												</wpsf:submit></li>

											<li><wpsa:actionParam action="removeStep"
													var="actionName">
													<wpsa:actionSubParam name="stepCode" value="%{#step.code}" />
												</wpsa:actionParam> <wpsf:submit action="%{#actionName}"
													cssClass="btn btn-warning" type="button"
													value="%{getText('label.remove')}"
													title="%{getText('label.remove')+': '+#step.code}">
													<span><s:text name="label.remove" /></span>
													<span class="sr-only"><s:property
															value="%{getText('label.remove')+': '+#step.code}" /></span>
												</wpsf:submit></li>
										</ul>
									</div>
								</div>
                ...
                <tr>
                    ...
                    <td><s:text name="name.contentStatus.PUBLIC" /></td>
                    <td><span class="sr-only">none</span>&ndash;</td>
                </tr>
                --%>
			</tbody>
			...
			<div class="form-group">
				<div class="col-xs-12">
					<label class="col-sm-2 control-label"
						for="jpcontentworkflow_stepCode"><s:text name="label.code" /></label>
					<div class="col-sm-10">
						<wpsf:textfield maxlength="12" name="stepCode"
							id="jpcontentworkflow_stepCode" cssClass="form-control" />
					</div>
				</div>
			</div>
			<div class="form-group">
				<div class="col-xs-12">
					<label class="col-sm-2 control-label"
						for="jpcontentworkflow_stepDescr"><s:text
							name="label.descr" /></label>
					<div class="col-sm-10">
						<wpsf:textfield name="stepDescr" id="jpcontentworkflow_stepDescr"
				...
				<div class="col-xs-12">
					...
					<div class="col-sm-10">
						<div class="input-group">
							...
							<span class="input-group-btn"> <wpsa:actionParam
									action="addStep" var="actionName" /> <wpsf:submit
									type="button" action="%{#actionName}" cssClass="btn btn-info">
