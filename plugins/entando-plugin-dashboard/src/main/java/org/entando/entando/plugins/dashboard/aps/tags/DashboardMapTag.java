...
public class DashboardMapTag extends TagSupport {

	private static final Logger _logger =  LoggerFactory.getLogger(DashboardMapTag.class);
	
	@Override
	public int doStartTag() throws JspException {
		ServletRequest request =  this.pageContext.getRequest();
		IDashboardMapManager dashboardMapManager = (IDashboardMapManager) ApsWebApplicationUtils.getBean("dashboardDashboardMapManager", this.pageContext);
		RequestContext reqCtx = (RequestContext) request.getAttribute(RequestContext.REQCTX);
		try {
		DashboardMap dashboardMap = null;
			if (null != this.getKey()) {
				dashboardMap = dashboardMapManager.getDashboardMap(this.getKey());
			} else {
				Widget widget = this.extractWidget(reqCtx);
				ApsProperties widgetConfig = widget.getConfig();
				String varid = widgetConfig.getProperty("id");
				if (StringUtils.isNotBlank(varid)) {
					dashboardMap = dashboardMapManager.getDashboardMap(new Integer(varid));
				}
			}
			this.pageContext.setAttribute(this.getVar(), dashboardMap);
		} catch (Throwable t) {
			_logger.error("Error in doStartTag", t);
			throw new JspException("Error in DashboardMapTag doStartTag", t);
		}
