...
<ol class="breadcrumb page-tabs-header breadcrumb-position">
	...
	<s:if test="getStrutsAction() == 2">
		<li class="page-title-container"><s:text
				name="title.jpsurvey.question.edit" /></li>
	</s:if>
	<s:if test="getStrutsAction() == 1">
		<li class="page-title-container"><s:text
				name="title.jpsurvey.question.new" /></li>
			...
			<h1>
				<s:if test="getStrutsAction() == 2">
					...
					<span class="pull-right"> <a tabindex="0" role="button"
						...
						title=""
						data-content="<s:text name="title.jpsurvey.question.edit.help" />"
				...
				<s:if test="getStrutsAction() == 1">
					...
					<span class="pull-right"> <a tabindex="0" role="button"
						data-toggle="popover" data-trigger="focus" data-html="true"
						title=""
						data-content="<s:text name="title.jpsurvey.question.new.help" />"
						data-placement="left" data-original-title=""> <i
							class="fa fa-question-circle-o" aria-hidden="true"></i>
					</a> <s:set var="isModifiable" value="true" />
				</s:if>
				</span>
			</h1>
		</div>
...
<div id="main">
	<div class="panel panel-default">
		<div class="panel-body">
			...
			:&#32; <em><s:property
					value="%{getLabel(question.surveyTitles)}" /></em>
		</div>
	</div>
						...
						<s:iterator var="cyclingLang" value="langs" status="langStatusVar">
							<div id="<s:property value="#cyclingLang.code" />_tab"
								...
								<label class="col-sm-2 control-label"
									for="question-<s:property value="#lang.code" />"><s:text
										name="jpsurvey_question" /></label>
								<s:set var="localization"
									value="%{getQuestions()[#cyclingLang.code]}" />
								<div class="col-sm-10">
									<wpsf:textarea name="%{'question-'+#cyclingLang.code}"
										id="%{'question-'+#cyclingLang.code}" value="%{#localization}"
										cssClass="form-control" cols="60" rows="3" />
								</div>
							</div>
						</s:iterator>
					</div>
		...
		<s:if test="%{strutsAction == 2}">
			<fieldset class="col-xs-12">
				<div class="form-group">
					<label class="col-sm-2 control-label" for="singleChoice"><s:text
							name="jpsurvey_singleChoice" /></label>
					<div class="col-sm-10">
						<select name="singleChoice" id="singleChoice"
							tabindex="<wpsa:counter />" class="form-control">
							<option value="1"
								<s:if test="%{singleChoice == null || singleChoice > 0}">selected="selected"</s:if>><s:text
									name="jpsurvey_isSingleChoice" /></option>
							<option value="0"
								<s:if test="%{singleChoice == 0}">selected="selected"</s:if>><s:text
									name="jpsurvey_notSingleChoice" /></option>
						</select>
					</div>
					<br>
				</div>
			...
			<fieldset class="col-xs-12">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label" for="minResponseNumber"><s:text
							name="jpsurvey_minResponseNumber" /></label>
					<div class="col-sm-10">
						<wpsf:textfield name="minResponseNumber" id="minResponseNumber"
							cssClass="form-control" />
					</div>
					<br>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label" for="maxResponseNumber"><s:text
							name="jpsurvey_maxResponseNumber" /></label>
					<div class="col-sm-10">
						<wpsf:textfield name="maxResponseNumber" id="maxResponseNumber"
							cssClass="form-control" />
					</div>
					<br>
				</div>
					...
					<div class="form-group">
						<div class="btn-group pull-right">
							<wpsa:actionParam action="addChoice" var="addChoiceAction">
								<wpsa:actionSubParam name="overrideResponseNumberFieldsCheck"
									value="true"></wpsa:actionSubParam>
							</wpsa:actionParam>
							<wpsf:submit type="button" action="%{#addChoiceAction}"
							...
							<s:if test="question.questionnaire">
								...
								<wpsf:submit type="button" action="%{#addFreeTextAction}"
									value="%{getText('jpsurvey_new_freeText')}"
									cssClass="btn btn-default" />
							</s:if>
						</div>
						<br>
						...
						<thead>
							<tr>
								<th class="text-center"><s:text
										name="jpsurvey_answersTable_answers" /></th>
								<s:if test="%{!#isModifiable}">
									<th class="text-center"><s:text
											name="jpsurvey_answersTable_occurences"></s:text></th>
								</s:if>
								<th class="text-center"><s:text
										name="jpsurvey_answersTable_actions" /></th>
							</tr>
						</thead>
						<tbody>
							<s:iterator var="currentChoice" value="question.choices"
								...
								<tr class="text-center">
									...
									<s:if test="%{#currentChoice.freeText== false}">
										<s:set var="localizedString"
											value="%{getText('jpsurvey_freeText')}" />
									</s:if>
									<s:else>
										<s:set var="localizedString"
											value="%{getText('jpsurvey_freeText')}" />
									...
									<s:if test="%{#currentChoice.freeText== false}">
										<td><s:property
												value="%{getLabel(#currentChoice.choices)}" /></td>
									</s:if>
									<s:else>
										<td><s:text name="jpsurvey_freeText" /></td>
									</s:else>
									<td><s:if
											test="%{#currentStat == '' || #currentStat==null}">
                                    0
                                </s:if> <s:else>
									...
									<td class="table-view-pf-actions">
										<div class="dropdown dropdown-kebab-pf">
                                           ...
                                           <s:param name="strutsAction" value="2"/>
												...
												<li><a
													href="<s:url action="moveChoiceUp">
                                       <s:param name="choiceId" value="#currentChoice.id"/>
                                       <s:param name="questionId" value="question.id"/>
												...
												<li><a
													href="<s:url action="moveChoiceDown">
                                   <s:param name="choiceId" value="#currentChoice.id"/>
                                   <s:param name="questionId" value="question.id"/>
                               </s:url>"
