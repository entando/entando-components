...
<s:if test="%{#currentRoot.getChildrenCodes().length==0}">
    ...
    class="treeRow <s:if test="%{#isHidden}">collapsed childrenNodes</s:if> tree_node_flag ${liClassName}" >
        <td class="treegrid-node pointer">
            <input type="radio" name="${inputFieldName}" 
                ...
                class="subTreeToggler <s:if test="#isSelected">active </s:if> hidden<s:if test="#currentRoot.childrenCodes.length > 0">  tree_<s:property value="#currentRoot.code" /> </s:if>"
...
<s:if test="%{#currentRoot.getChildrenCodes().length>0}">
    <s:iterator value="#currentRoot.childrenCodes" var="node">
        <s:set var="currentRoot" value="%{getCategory(#node)}" />
        <s:include value="/WEB-INF/plugins/jpcontentscheduler/apsadmin/jsp/config/treeBuilderCategoriesJoin.jsp" ></s:include>
    </s:iterator>
</s:if>